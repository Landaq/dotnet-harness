...
## Codex workflow modes

Codex는 사용자 요청을 다음 세 가지 작업 방식 중 하나로 분류하고, 작업 방식별 모호도 임계값을 만족한 뒤 구현을 시작한다. 모호도는 Codex가 임의로 추론해야 하는 요구사항의 비율을 의미하며, 질문을 할 때마다 현재 수치를 표시하고 사용자의 답변을 받은 뒤 다시 계산한다.

| 작업 방식 | 적용 대상 | 구현 가능 모호도 | 핵심 운영 규칙 |
| --- | --- | ---: | --- |
| 복잡한 작업 | 풀스택 기능 추가, 다중 서비스 변경, 기능 추가와 리팩토링, 아키텍처 변경 | **13% 이하** | 서비스 경계, API 계약, Gateway, Aspire, FrontEnd, 테스트 계층을 함께 검토한다. |
| 백엔드 작업 | `src/BackEnd`, `src/Aspire`, `APIGateway`, EF Core, MS SQL Server, Minimal API 중심의 간소한 수정·추가 | **5% 이하** | Domain/Application/Infrastructure/Api/Contracts 경계, EF Core 영향, 보안, 계약 테스트를 우선 확인한다. |
| 프론트엔드 작업 | `src/FrontEnd/Web`, `src/FrontEnd/Web.Client`, Blazor, MudBlazor, DevExpress BI 대비 중심의 간소한 수정·추가 | **5% 이하** | Auto Rendering, `Web.Client` 브라우저 안전성, MudBlazor 우선, DevExpress Blazor 23.2 BI 기준, API 계약을 확인한다. |

요구사항 구체화 질문은 소크라테스식으로 작성한다. 한 번에 최대 **3개 안건**만 `1.`, `2.`, `3.` 형식으로 질문하며, Codex가 권장하는 선택지는 반드시 **`(추천)`**으로 표시한다. 모호도가 해당 작업 방식의 임계값을 초과하면 구현하지 않고 추가 질문 또는 계획 보완을 수행한다.

공통 워크플로는 `요청사항 구체화 → Git 브랜치 생성 및 worktree 분리 → 계획 수립 → 스펙 산정 → .md 파일 생성 → 코드 구성 → 리뷰 및 피드백 → 피드백 반영 → Git 머지 후 병합 worktree 제거 → 결과 .html 생성` 순서를 따른다. 스펙 파일은 `docs/wkTask/Specs/{yyMMdd}_{Summary}_plan.md`, 결과 파일은 `docs/wkTask/Results/{yyMMdd}_{Summary}_result.html` 형식으로 작성한다. 스펙 산정 후에는 사용자 검토 승인을 받아야 하며, 사용자가 기존 plan 파일을 명시하면 코드 구성 단계부터 바로 진행한다.

Git worktree 분리, 브랜치 생성, 커밋, 푸시, 머지, worktree 제거, 파일 삭제, reset, clean 같은 파괴적 작업은 사용자의 명시 승인 후에만 수행한다. 민감 정보, 연결 문자열, 토큰, 인증서, 개인 키, 배포 자격 증명은 워크플로 산출물과 응답에 포함하지 않는다. 상세 절차는 `docs/architecture/WORKFLOW_GUIDE.md`를 기준으로 한다.
...
| `docs/architecture/WORKFLOW_GUIDE.md` | 복잡한 작업·백엔드 작업·프론트엔드 작업의 Codex 워크플로, 모호도 임계값, 스펙·결과 산출물 규칙 |
